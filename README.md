# ![https://github.com/brhestir/GBSearch/blob/main/README.md](https://badgen.net/badge/GB/search/yellow?icon=github&scale=5)

Search within Google Books for matching items. Publish interesting results for later research, notifying subscribed peers via SocketIO. Hastily composed with React.

![https://github.com/brhestir/GBSearch/blob/main/LICENSE](https://shields.io/github/license/brhestir/GBSearch)

## Table of Contents

- [Description](#description)
- [Installation](#installation)
- [Usage](#usage)
- [Licence](#licence)
- [Contributing](#contributing)
- [Questions](#questions)

## Description

Search books matching terms using Google Books Search API; direct attention to selected materials.

[GitHub Repo](https://www.github.com/brhestir/GBSearch)

[Deployed Heroku Instance](https://still-shore-47010.herokuapp.com/)

![](./assets/images/GBSearch.png)

## Installation

```bash
npm install
```

## Usage

```bash
npm start
```

## Licence

![https://github.com/brhestir/GBsearch/blob/main/LICENSE](https://shields.io/github/license/brhestir/GBsearch)

## Contributing

Clone this repo and create a pull request.

## Questions

Questions/comments/suggestions welcome.
